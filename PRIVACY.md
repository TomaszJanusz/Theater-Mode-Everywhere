...
Theater mode may request caption files or timeline preview images **directly from the site you are already watching** (for example YouTube caption tracks, YouTube storyboard sprites, Vimeo `vimeocdn` thumbnail sprites, Patreon/Mux `image.mux.com` storyboard files, Patreon/Mux `stream.mux.com` caption WebVTT files, or a page's own WebVTT/SRT files already referenced by a `<track>` element). Those requests stay between your browser and that provider. Captions and preview images are kept only in memory for the current playback session and are never stored, synced, or sent to us. You can turn YouTube, Vimeo, and Patreon extras off in Settings → Features; theater mode itself still works on those sites.
...
- **Provider integrations** — whether extra YouTube, Vimeo, and Patreon theater features (captions, chapters, previews) are enabled. These default to on.
