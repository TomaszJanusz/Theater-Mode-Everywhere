...
Theater mode may request caption files or timeline preview images **directly from the site you are already watching** (for example YouTube caption tracks or storyboard sprites already referenced by the player). Those requests stay between your browser and that provider. Captions and preview images are kept only in memory for the current playback session and are never stored, synced, or sent to us. You can turn YouTube and Vimeo extras off in Settings → Features; theater mode itself still works on those sites.
