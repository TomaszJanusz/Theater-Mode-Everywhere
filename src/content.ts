...
function createCustomControls(video: HTMLVideoElement): void {
  ...
  timeDisplay.addEventListener('click', (e) => {
    e.stopPropagation();
    if (playbackWindow(video).live) return;
  ...
  const syncLiveChrome = () => {
    ...
    timeDisplay.style.cursor = window.live ? 'default' : 'pointer';
  ...
  const updateTimeDisplay = () => {
    ...
    if (window.live) {
      ...
      if (!window.seekable || isAtLiveEdge(cur, window) || behind > MAX_LIVE_DVR_SECONDS) {
